...
face_ave = read_reshape_blur_image('average_face.jpg', 224, 5)
face_ref = face_ave.reshape((1,224,224,3))
face_aj = read_reshape_blur_image('aj2.jpg', 224, 5)

face_ave_sharpen = sharpen_gray_image(face_ave, 30)
face_aj_sharpen = sharpen_gray_image(face_aj, 30)
...
class GG(object):
	...
	def __init__(self, sess, input_dim, model_url, model0_url, batch_size=1, learning_target=0, 
	...
	def discriminator(self, image):
		with tf.variable_scope('Discriminator') as scope:
			# load 'facedetect.tfmodel'
			if 'face' in self.model0_url:
				with open(self.model0_url, mode='rb') as f:
					model = f.read()
				graph_def = tf.GraphDef()
				graph_def.ParseFromString(model)
				image_rev = tf.reverse(image, [-1])
				tf.import_graph_def(graph_def, input_map={'images': image_rev, 'Deepnn/keep_prob': 1.0})
				return tf.nn.sigmoid(tf.get_default_graph().get_tensor_by_name('Discriminator/import/Deepnn/prob:0') - tf.constant(600.))
	...
	def build_model(self):
		...
		self.loss_l2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D0, labels=tf.ones_like(self.D0)))
...
#		self.saver = tf.train.Saver()
	...
	def train(self):
		...
		data_input_noise = np.random.uniform(-1, 1, [self.batch_size,self.input_dim]).astype(np.float32)
...
with tf.Session() as sess:
	...
	if 'face' in gnn.model_url:
		misc.imsave('face_image.jpg', gnn.pic[0])
		name = [s.strip() for s in open('names.txt').readlines()]
		rank = np.argsort(gnn.prob[0])[::-1]
		print "top 1:", name[rank[0]], gnn.prob[0][rank[0]]
		print "top 2:", name[rank[1]], gnn.prob[0][rank[1]]
		print "top 3:", name[rank[2]], gnn.prob[0][rank[2]]
		...
		plt.imshow(face_aj)
		f.add_subplot(2,3,2)
		plt.imshow(gnn.pic[0])
		f.add_subplot(2,3,3)
		plt.imshow(face_ave)
		f.add_subplot(2,3,4)
		plt.imshow(face_aj_sharpen, cmap='gray')
		f.add_subplot(2,3,5)
		plt.imshow(sharpen_gray_image(gnn.pic[0], 30), cmap='gray')
		f.add_subplot(2,3,6)
		plt.imshow(face_ave_sharpen, cmap='gray')
		plt.show()
	else:
		print 'label:', gnn.prob[0]
		np.savetxt('number_image.txt', gnn.pic[0])
		f = plt.figure()
		for i in range(9):
			f.add_subplot(3,3,i+1)
			plt.imshow(gnn.pic[i*5,:,:,0], cmap='gray')
		plt.show()
